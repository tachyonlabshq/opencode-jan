...
export class ModelStatusCache {
  ...
  async getModels(baseURL: string, fetchFn: () => Promise<string[]>): Promise<string[]> {
    ...
    try {
      ...
      if (cached) {
        console.warn(`[opencode-jan] Using stale cache data due to fetch error`, { 
          baseURL, 
  ...
  invalidate(baseURL: string): void {
    ...
    console.debug(`[opencode-jan:DEBUG] Invalidated cache entry`, { baseURL })
  }
  ...
  invalidateAll(): void {
    ...
    this.cache.clear()
    console.debug(`[opencode-jan:DEBUG] Cleared entire cache`, { previousSize: size })
  }
  ...
  private cleanup(): void {
    ...
    if (toDelete.length > 0) {
      console.debug(`[opencode-jan:DEBUG] Cleaned up cache entries`, { deleted: toDelete.length, remaining: this.cache.size })
    }
  }
  ...
  setTTL(baseURL: string, ttl: number): void {
    ...
    if (cached) {
      cached.ttl = ttl
      console.debug(`[opencode-jan:DEBUG] Updated TTL for cache entry`, { baseURL, ttl })
    }
  }
